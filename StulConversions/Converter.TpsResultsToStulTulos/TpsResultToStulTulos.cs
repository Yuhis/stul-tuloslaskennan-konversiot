...
using Files.Stul.TulosTxt;
using Files.Tps.ResultsXml;
using System;
...
{
    public class TpsResultToStulTulos
    {
        private readonly StulParitReader _stulParitReader;
        private readonly TpsResultsReader _tpsResultsReader;
        private readonly string _stulTulosFileName;
        private readonly IDictionary<string, TpsEvent> _tpsEvents;

        public TpsResultToStulTulos(string stulParitFileName, string tpsResultsFileName, string stulTulosFileName)
        {
            // parit.txt tiedosto
            ...
            foreach (TpsResultsElement element in _tpsResultsReader.Event.Results)
            {
                ...
                if (null == tpsCompetition)
                {
                    tpsCompetition = new TpsCompetition(element.CompetitionCode, tpsCompetitionType)
                    {
                        ...
                        TotalCouples = element.TotalCouples
                    };
                ...
                {
                    CoupleNumber = element.CoupleNumber,
                    CoupleNames = element.CoupleNames,
                    Position1 = element.Position1,
                    Position2 = element.Position2,
                    Missing = tpsMissing,
                    RoundsDanced = element.RoundsDanced
                };
